<?php 
			$accao = 'readMenu'; require 'controller.php'; 
...
<body class="bg-secondary">
	...
	<header>
		<nav class="navbar navbar-expand-sm navbar-dark bg-dark">
			...
			<!--Inicio do Usuário logado ao sistema-->
	    ...
	    </ul><!--Fim do Usuário logado ao sistema-->
	...
	<section>	
		<div class="row mx-0 my-2"><!--Inicio da Row-->
			...
			<div class="col-sm-2 mb-5 mt-5">
				...
				<div class="mt-2 mb-2">
					<a href="usuario.php" class="btn btn-warning btn-block ml-2 font-weight-bold text-dark"><i class="fas fa-user-edit mr-2"></i>Usuario</a></div><!--Fim do botão Usuario-->
				...
				<div class="mt-2 mb-2">
					<a href="menu.php" class="btn btn-warning btn-block ml-2 font-weight-bold text-dark active"><i class="fas fa-book-open mr-2"></i>Menu</a>
			...
			<div class="col-sm-3 mb-5 mt-3 border rounded bg-dark border-warning">
				...
				<h3 class="text-center text-white mb-2 mt-2 border-bottom border-warning">Cadastrar Menu</h3>

				<!--Condição para buscar resgistro na BD para a edição do mesmo-->
				<?php if (isset($_GET['idup'])) {
							$idupdate = $_GET['idup'];
							$menu = $service->pesquisarMenu($idupdate); }	?>

				<div class=" form-group text-white"><!--Inicio do Div Formulario-->

					<form action="<?php if(isset($menu)) { ?> controller.php?accao=updateMenu <?php } else{ ?>controller.php?accao=creatMenu<?php } ?>" method="POST">

						<label for="produto" class="text-white font-weight-bold">Categoria</label>
						<select name="produto" class="form-control mb-2 border border-warning" id="produto" >
							<option selected>Selecione a categoria</option>
							<?php //Condição para consultar a informação da bd na Cbox 
											
								$Pmenu = $service->pesquisarProduto();

								foreach ($Pmenu as $rows)
								{
									echo "<option value=".$rows->idproduto.">".$rows->nome."</option>";
								}
								
							?> 
						</select>

						<label for="menu" class="text-white font-weight-bold">Nome do prato</label>
						<input type="text" name="menu" class="form-control mb-2 border border-warning" id="menu" placeholder="Digite o nome do prato" value="<?php if(isset($menu)){echo $menu['nome_menu'];} ?>">

						<label for="preco" class="text-white font-weight-bold">Preço</label>
						<input type="text" name="preco" class="form-control mb-2 border border-warning" id="preco" placeholder="Digite o preço do prato" value="<?php if(isset($menu)){echo $menu['preco'];} ?>">

						<label for="desc" class="text-white font-weight-bold">Descrição do prato</label>
						<input type="text" name="desc" class="form-control mb-3 border border-warning" id="desc" placeholder="Ex: Salada fria com ovos e atum enlatado " value="<?php if(isset($menu)){echo $menu['descricao'];} ?>">

						<input type="hidden" name="id" value="<?php if(isset($menu)){echo $menu['iditem'];} ?>">

						<!--Validação em php para notificar registros cadastrado com sucesso-->
	          ...
	          <div class="text-white text-justify text-center mt-0 mb-0 bg-success">
	        	...
	        	<?php } ?>
						...
						<input type="submit" class="btn btn-warning btn-block mt-4 mb-5 font-weight-bold" value="<?php if(isset($menu)){echo 'Actualizar';}else{echo 'Cadastrar';} ?>">
			...
			<div class="col-sm-7 mb-5 mt-3 border rounded bg-dark border-warning">
				<div class="table-responsive"><!--Inicio do Div Tabela Responsive-->
					...
					<table class="table table-sm text-white"><!--Inicio da Tabela -->
						...
						<tbody>
							<?php foreach ($readMenu as $menu) { ?>
							<tr>

								<td id="menu_<?= $menu->iditem ?>">
									<?= $menu->nome ?>
								</td>
								<td id="menu_<?= $menu->iditem ?>">
									<?= $menu->nome_menu ?>
								...
								<td>
									<a href="menu.php?idup=<?= $menu->iditem ?>"><i class="fas fa-edit text-info"></i></a>
								</td>
								<td>
									<a href="controller.php?accao=deleteMenu&id=<?= $menu->iditem ?>"><i class="fas fa-window-close text-danger"></i></a>
								</td>
							</tr>
							<?php } ?>
						</tbody>
