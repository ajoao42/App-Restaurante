<?php 
			$accao = 'readReserva';	require 'controller.php'; 
...
<head>
	...
	<title>Restaurante H. Miradouro - Reserva</title>
	<!--Bootstrap-->
...
<body class="bg-secondary">
		...
		<section>
			<div class="row mx-0 my-2"><!--Inicio da Row-->
				...
				<div class="col-sm-2 mb-5 mt-5">
					...
					<div class="mt-2 mb-2">
						<a href="reserva_User.php" class="btn btn-warning btn-block ml-2 font-weight-bold text-dark active"><i class="fas fa-list-alt mr-2"></i>Reserva</a>
				...
				<div class="col-sm-3 mb-5 mt-3 border rounded bg-dark border-warning">
					<h3 class="text-center text-white mb-2 mt-2 border-bottom border-warning">Fazer Reserva</h3>

					<!--Condição para ir buscar os registros na BD para actualização-->
					<?php if (isset($_GET['idup'])) {
								$idupdate = $_GET['idup'];
								$res = $service->pesquisarReserva($idupdate); }	?>

					<div class=" form-group text-white"><!--Inicio da Div formulario-->

						<form action="<?php if(isset($res)) { ?> controller.php?accao=updateReserva <?php } else{ ?> controller.php?accao=creatReserva <?php } ?>" method="POST">
							...
							<select name="cliente" class="form-control mb-2 border border-warning" id="cliente" >
								...
								<?php //Validação para buscar a informação na bd e mostrar na Cbox
							...
							<label for="data" class="text-white font-weight-bold">Data da Reserva</label>
							<input type="date" name="data" class="form-control mb-2 border border-warning" id="data" value="<?php if(isset($res)){echo $res['data'];} ?>">

							<label for="hora" class="text-white font-weight-bold">Hora da Reserva</label>
							<input type="time" name="hora" class="form-control mb-2 border border-warning" id="hora" value="<?php if(isset($res)){echo $res['hora'];} ?>">

							<label for="lugares" class="text-white font-weight-bold">Lugares Reservados</label>
							<input type="text" name="lugares" class="form-control mb-3 border border-warning" id="lugares" placeholder="Ex: 6 Lugares" value="<?php if(isset($res)){echo $res['n_lugares'];} ?>">

							<input type="hidden" name="id" value="<?php if(isset($res)){echo $res['idreserva'];} ?>">

							<!--Validação em php para notificar registro cadastrado com sucesso-->
							<?php if (isset($_GET['inclusao']) && $_GET['inclusao'] == 'save') { ?>

		          <div class="text-white text-justify text-center mt-0 mb-1 bg-success">
		          ...
		          <div class="text-white text-justify text-center mt-0 mb-1 bg-info">
		          ...
		          <div class="text-white text-justify text-center mt-0 mb-1 bg-danger">
		        	...
		        	<?php if (isset($_GET['inclusao']) && $_GET['inclusao'] == 'erro1') { ?>
		          	...
		          	<div class="text-white text-justify text-center mt-0 mb-1 bg-warning">
		           	 Preenche todos campos para o cadastramento 
		          	</div>

		        	<?php } ?>

							<input type="submit" class="btn btn-warning btn-block mt-4 mb-5 font-weight-bold" value="<?php if(isset($res)){echo 'Actualizar';}else{echo 'Reservar';} ?>">
							...
							<table class="table table-sm text-white"><!--Inicio da tabela-->
								<thead class="table-dark text-center">
									<tr>
										...
										<th>Lugares Reservado</th>
										<th colspan="2"></th>
									</tr>
								</thead>

								<tbody class="text-center">
									<?php foreach ($readReserva as $reserva) { ?>
										<tr>
											...
											<td>
												<a href="reserva_User.php?idup=<?= $reserva->idreserva ?>"><i class="fas fa-edit text-info"></i>
												</a>
											</td>
											<td>
												<a href="controller.php?accao=deleteReserva&id=<?= $reserva->idreserva ?>"><i class="fas fa-window-close text-danger"></i>
												</a>
											</td>
										</tr>
									<?php } ?>
	...
	<!--Inicio do Rodapé da App-->
		<footer>
			<div class="bg-dark text-white pt-2 pb-2 pl-2 pr-3" style="margin-top: 223px;">
				<div class="row">
					<div class="col-sm-9">
